...
import { X, ArrowLeft, PlusCircle, Upload, ImageIcon, Trash2 } from "lucide-react";
  ...
  return (
    <div className="p-6 lg:p-10 max-w-3xl mx-auto">
      <div className="pb-6 border-b border-slate-100 mb-8">
        ...
        <h1 className="text-2xl font-black tracking-tight text-slate-900 mb-1">Yangi Kurs Yaratish</h1>
        <p className="text-slate-500 text-sm font-medium">Tasdiqlash uchun adminlarga yangi kurs jo'nating.</p>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-5 bg-white border border-slate-100 shadow-sm p-6 md:p-8 rounded-2xl">
        <div className="space-y-1.5">
          ...
          <input required type="text" value={title} onChange={e => setTitle(e.target.value)} className={inputCls} placeholder="Kengaytirilgan Node.js darslari" />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <label className="text-xs font-black uppercase tracking-widest text-slate-500">Kategoriya</label>
            <input required type="text" value={categoryName} onChange={e => setCategoryName(e.target.value)} className={inputCls} placeholder="Dasturlash" />
          </div>
          <div className="space-y-1.5">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <input type="text" value={techInput} onChange={e => setTechInput(e.target.value)}
              ...
              className={`${inputCls} flex-1`} placeholder="JavaScript, React, Node.js..." />
            <Button type="button" onClick={addTech} className="h-11 px-5 rounded-xl font-black text-xs uppercase tracking-widest bg-blue-600 hover:bg-blue-700 text-white border-0 shrink-0">
              + Qo'shish
            </Button>
          </div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {technologies.map(t => (
                <span key={t} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 text-blue-600 border border-blue-100 text-xs font-bold">
                  {t}
                  <button type="button" onClick={() => setTechnologies(technologies.filter(x => x !== t))} className="text-blue-400 hover:text-blue-700">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
        ...
        <div className="space-y-1.5">
          ...
          <textarea required value={description} onChange={e => setDescription(e.target.value)}
            ...
            placeholder="Bu kurs orqali siz..." />
        </div>
