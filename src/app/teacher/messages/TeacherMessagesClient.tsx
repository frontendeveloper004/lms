...
export function TeacherMessagesClient({ currentUserId }: TeacherMessagesClientProps) {
  ...
  >(new Map());
  const [selectedStudent, setSelectedStudent] = useState<ChatUser | null>(null);
  const [loading, setLoading] = useState(true);
  // Mobile: "list" | "chat"
  const [mobileView, setMobileView] = useState<"list" | "chat">("list");

  const fetchData = useCallback(async () => {
    const [studentsRes, convsRes] = await Promise.all([
      fetch("/api/teacher/chat-students"),
      fetch("/api/messages/conversations"),
    ]);
    const studentsData: ChatUser[] = studentsRes.ok ? await studentsRes.json() : [];
    const convsData: {
      partner: ChatUser;
  ...
  useEffect(() => {
    if (!chatWithId || loading || students.length === 0) return;
    const target = students.find((s) => s.id === chatWithId);
    if (target && (!selectedStudent || selectedStudent.id !== chatWithId)) {
      setSelectedStudent(target);
      setMobileView("chat");
    }
  }, [chatWithId, loading, students, selectedStudent]);

  const enrichedStudents: ChatUser[] = students.map((s) => {
  ...
  const handleSelectStudent = (student: ChatUser) => {
    setSelectedStudent(student);
    setMobileView("chat");
    setConversations((prev) => {
      const next = new Map(prev);
      const existing = next.get(student.id);
      if (existing) next.set(student.id, { ...existing, unreadCount: 0 });
      return next;
    });
  };
  ...
  const handleBack = () => {
    ...
    router.replace("/teacher/messages", { scroll: false });
  };

  const totalUnread = enrichedStudents.reduce(
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3 px-6 text-center">
          ...
          <p className="text-slate-500 text-sm font-bold">Talabalar yo'q</p>
          <p className="text-slate-400 text-xs font-medium leading-relaxed">
            Kurslaringizga talabalar qo'shilgach, ular bu yerda ko'rinadi
          </p>
        </div>
      ) : (
        <ConversationList
  ...
  const chatPanel = selectedStudent ? (
    <ChatWindow
      ...
      currentUserId={currentUserId}
      partner={{ ...selectedStudent, role: "STUDENT" }}
      onBack={handleBack}
    />
  ) : (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 bg-slate-50/50">
      ...
      <div className="text-center">
        ...
        <p className="text-slate-400 text-sm font-medium mt-1">
          Chap paneldan talabani tanlang va muloqotni boshlang
        </p>
      </div>
    </div>
  );
